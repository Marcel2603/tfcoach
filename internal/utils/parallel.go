...
func FlatMap[IN any, OUT any](
	...
	return FlatMapChan(inArr, func(in IN, outChan chan<- OUT) {
...
// FlatMapChan See FlatMap, but the action performed on each element sends its output onto an open
// channel. Prefer this variant if said channel is being written to by other goroutines.
